...
const VIBE_ORDER: Vibe[] = ["going_well", "watch_it", "stuck", "quiet_week"];
    ...
    <div className="flex flex-col gap-3.5">
      ...
      {awaiting.length > 0 && (
        <div className="rounded-card border border-dashed border-sand-300 bg-cream/60 px-4 py-3 flex flex-wrap items-center gap-2">
          ...
          {awaiting.map(({ programme, freshness }) => (
            <Link
              key={programme.id}
...
function MoodShelf({ vibe, entries }: { vibe: Vibe; entries: BoardEntry[] }) {
